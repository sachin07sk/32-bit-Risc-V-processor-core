// ============================================
// RISC-V RV32I — Top Level Module
// Connects all 12 submodules with internal wires
...
// ══════════════════════════════════════════════
// WIRE DECLARATIONS — every signal between modules
// ══════════════════════════════════════════════

// ── PC wires ──────────────────────────────────
wire [31:0] pc_current;       // PC → instr_mem, if_stage
wire [31:0] branch_target;    // mem_stage → pc
wire        branch_taken;     // mem_stage → pc, hazard_unit
wire        pc_stall;         // hazard_unit → pc

// ── Instruction memory wires ──────────────────
wire [31:0] instruction;      // instr_mem → if_stage
...
// ── Register file wires ───────────────────────
wire [4:0]  rs1, rs2;         // id_stage → reg_file, hazard_unit
wire [31:0] read_data1;       // reg_file → id_stage
wire [31:0] read_data2;       // reg_file → id_stage
wire [4:0]  wb_rd;            // wb_stage → reg_file
wire        wb_reg_write;     // wb_stage → reg_file
wire [31:0] wb_write_data;    // wb_stage → reg_file, forwarding_unit
...
// ── PC select ────────────────────────────────
wire        pc_sel = branch_taken;  // 1=branch target, 0=PC+4
